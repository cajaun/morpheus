...
import type { ActionTrayLifecycleParams } from "./action-tray-lifecycle-types";
import {
  resolveActionTrayCloseTravel,
  resolveActionTrayOpenTravel,
} from "./action-tray-lifecycle-motion";

type Params = ActionTrayLifecycleParams;
...
export const useActionTrayOpenCloseLifecycle = ({
  ...
  const doOpenSpring = useCallback(() => {
    ...
    const openTravel = resolveActionTrayOpenTravel(
      resolveClosedTranslateY,
      nextFooterHeight,
      nextContentHeight,
    );
  ...
  useLayoutEffect(() => {
    if (visible) {
      ...
      const closeTravel = resolveActionTrayCloseTravel(
        resolveClosedTranslateY,
